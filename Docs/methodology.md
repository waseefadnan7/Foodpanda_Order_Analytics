...
foodpanda's app does not offer a native way to export order history. To get the data out, I built a small script to pull my own data directly from the page: it read the already-loaded order history elements in the browser and saved the results as a CSV.

This only worked on my own account, my own already-authenticated session, and only captured what was already visible/loaded on the page after scrolling through the full history. No external requests, automation against foodpanda's servers, or access beyond my own account were involved.
